...
6. CI на GitHub — вкладка Actions.
7. PR → Merge.
8. Удалить ветку.
...
4. Source Control (`Ctrl+Shift+G`) → **один коммит** на все файлы →
   Commit & Push.
5. PR → CI (вкладка Actions) → Merge → Delete branch.
...
- `Run unit tests`.
- `Build Debug APK`.
- Артефакты: `app-debug`, `test-report`.

**Merge блокируется, пока CI красный.**

Vosk-модель не коммитится в Git. В CI выкачивается из релиза GitHub.
