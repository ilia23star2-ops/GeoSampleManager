# GeoSample Manager — статус проекта

> Обновляется после каждого захода.
> Последнее обновление: **после закрытия 5.8.9e-2-fix-3**.

---

## Как продолжить работу в новом чате

1. Прикрепи файлы:
   - `AI_RULES.md`
   - `CONTEXT_BRIEF.md` (в первую очередь!)
   - `README.md`
   - `PROGRESS.md` (этот)
   - `NEXT_STEPS.md`
   - `DECISIONS.md`
   - `DATABASE.md`
   - `ROADMAP.md`
   - `VOICE.md`
   - `GLOSSARY.md`
   - `ISSUES.md`
   - `TESTING.md`
2. Скажи: «Изучи файлы, продолжаем с этапа N, заход N.M».

---

## Статус одной строкой

**Этапы 1–4, 5.1, 5.5, 5.8.1–5.8.4, 5.8.8 (d–k), 5.8.9 (a–g-3),
5.8.9e-2 (+ fix-1/2/3), 5.11 — ✅. Спецификация ГП — ✅.
Следующий заход — 5.8.9e-3 (`keepScreenOn`).**

---

## Что сделано

### ✅ Этапы 1–4 — База проекта
Скелет, Drawer, 7 вкладок, навигация, Room-БД (6 таблиц, version=2),
настройки импорта, Excel-импорт.

### ✅ Этап 5.1 — Каркас сверки
Полный UI-экран, модели, состояние, undo/redo, 11 диалогов.

### ✅ Этап 5.5 — Заметки и фото
`PhotoStorage`, `SampleImageEntity`, миграция 1→2, `NotePhotoDialog`,
флаги `has_note` / `has_photo`.

### ✅ Этап 5.8.1 — Каркас голоса
Разрешения, `VoiceController`, `VoiceDialog`, кнопка микрофона.

### ✅ Спецификация ГП (`VOICE.md`)
Полная спецификация.

### ✅ Заход 5.11 — Множественный поиск (UI)
Группировка, подсветка, дебаунс, режимы поиска, все баги закрыты.

### ✅ Заходы 5.8.2–5.8.4 — Парсер, префиксы, команды
`VoiceDictionary`, `VoiceNumberParser`, `VoiceSegmenter`,
`VoicePrefixResolver`, `VoiceSearch`, `VoiceOrdinals`,
`VoiceCommand`, `VoiceCommandParser`. ~38 юнит-тестов зелёные.

### ✅ Заход 5.8.8 — Vosk + грамматика + TTS

Vosk-small-ru, грамматика, авто-режим отметок, TTS по парам цифр,
пауза Vosk во время речи, разрешения runtime.

### ✅ Заходы 5.8.8d–k — Финальные фиксы ГП

- `spellNumber`, `awaitingWeight`, вес для холостых.
- `VoiceCommandParser`: явная проверка `Sort` по «и».
- `VoiceNumberParser`: `distinct()` — без дубликатов.
- `VoiceSearch`: `FoundMany` при разных нарядах; суффикс ≥ 3 символов; порог 50.
- UI: неоднозначный ответ (жёлтый ⚠, баннер, подписи групп).
- Автопауза `awaitingContinue`; перечисление по «и»; TTS по парам.
- `analyzeMatch`: 1 группа → всегда `Unique`.

### ✅ Заход 5.8.9-infra — Правила работы с двух машин

Дополнены `AI_RULES.md` §20 и `README.md` — работа через
`github.dev` без IDE.

### ✅ Заход 5.8.9a — Модель `AnswerState` + `AnswerReason`

`data/voice/AnswerState.kt` — 4 состояния (OK / ATTENTION / ERROR /
IDLE), 8 причин, единый маппер. Заменён старый `MatchInfo`.

### ✅ Заход 5.8.9a-t — Тесты модели

`AnswerStateTest` (21 тест), `AnalyzeMatchTest` (15 тестов).

### ✅ Заход 5.8.9b — UI: индикатор ответа, заголовки, цвета

`AnswerStateColors.kt`. Индикатор — 3 строки. Заголовки — трёхстрочная
структура. Строгий цвет: один цвет на состояние, без оттенков.

### ✅ Заход 5.8.9c — Звуковая карта

`VoiceFeedback.kt`: `soundOk`, `soundAttention`, `soundError`.
Гвардия от серии (К3, К5).

### ✅ Заход 5.8.9d-1 — TTS окна ответа

`FoundOne` расширен полями `blanks` / `weightControls` / `postponed`.
Фраза с ненулевыми категориями.

### ✅ Заход 5.8.9bug-1 — `VoiceSearch`: суффикс ≥ 3, порог 50

Короткий кандидат («1», «24») больше не даёт 251 совпадение.

### ✅ Заход 5.8.9bug-2 — Селекторы не перезаписываются

`voiceSearch` не трогает `selectedArea` / `selectedOrder`.

### ✅ Заход 5.8.9bug-3 + fix-1…fix-5 — ВК в настройках наряда

- Полная пересборка ВК при сохранении (`applyWeightControlForOrder`).
- `SampleRow.serialNumber` — п/п из БД, не зависит от фильтров.
- Кнопка «Сбросить ВК» + undo (`UndoAction.BulkRowsChange`).
- Undo работает для операций из настроек наряда.
- **Раздельные «Применить»**: холостые и ВК — каждая своя кнопка.

### ✅ Заход 5.8.9e-1 — Свёрнутые группы + участок в заголовке

`effectiveGroupExpanded(id, isAttention)`. При ATTENTION группы
свёрнуты. Заголовок запроса — строка на каждый участок с номерами
нарядов.

### ✅ Заход 5.8.9e-4 — ГП: другой участок / другой наряд

`FoundOne.attentionReason`. `soundAttention` + автопауза +
фраза «Другой участок — X, наряд Y. Выберите на экране.»

### ✅ Заход 5.8.9e-5 — `VoiceOrdinals` + spell-out

Формы «третья/третий/третье/третью» добавлены явно. В `Message`
номера проговариваются через `spellOut`.

### ✅ Заход 5.8.9g-1 — Парсер: `MarkByNumbers` + `MarkAll`

`VoiceOrdinals.matchAll` — находит все порядковые слева направо.
«Пятая шестая седьмая» → `MarkByNumbers([5, 6, 7])`.
«Отметь все» → `MarkAll`.

### ✅ Заход 5.8.9g-2 — Выполнение множественной отметки

`MarkedMultiple`, `MarkedAll`. «Search → отметка» в активной сессии
(«восемь» = проба №8). «Отметить все» — все неотмеченные пробы
текущей скважины.

### ✅ Заход 5.8.9g-3 — Грамматика + сброс сессии

Явные слова «все / отметь / отметьте / отметить» в грамматике Vosk.
`voiceSession.clear()` при `NotFound`. Сброс `lastMarked*` при смене
скважины.

### ✅ Заход 5.8.9e-2 — Индикатор мультипоиска

По строке на каждый запрос в одиночном индикаторе. Значки 🟢/🟡/🔴
на каждый запрос (до 5). Строка с ответом: наряд + значение.

### ✅ Заход 5.8.9e-2-fix-1 — Убраны префиксы «Н» / «П»

Путали пользователя — приняли за «проба» / «Н». Формат стал
`7 · NV152601`.

### ✅ Заход 5.8.9e-2-fix-2 — Показываем то, что искали

`MatchedKind` (WELL / SAMPLE / NONE) в `MatchInfo` и `QuickAnswer`.
Определяется по точному совпадению `wellNumber` / `sampleNumber`.
Одиночный — с префиксами «скв.» / «проба», мульти — без.

### ✅ Заход 5.8.9e-2-fix-3 — «Наряд N» + фикс отмены корутин

- Одиночный индикатор: `Наряд 7 · скв. NV1526` (развёрнуто).
- Мульти: `7 · 1524` (коротко).
- `CancellationException` во всех `catch` — больше нет ложных
  Snackbar `StandaloneCoroutine was cancelled` при быстром вводе.

---

## Что в работе

(Пусто)

---

## Что впереди

| # | Заход | Заходов | Статус |
|---|---|---|---|
| 5.8.9e-3 | `keepScreenOn` | 1 | ⬜ |
| 5.8.9h | Унификация UI и ГП | 1 | ⬜ |
| 5.8.9f | Режимы SORT / SEARCH | 2 | ⬜ |
| 5.8.9d-2 | Отметка короче (порядковый + счётчик) | 1 | ⬜ |
| 5.8.9d-3 | Переспросы ВК / отложенных | 1 | ⬜ |
| 5.8.9i | Мимикрия произношения | 1 | ⬜ |
| 5.8.6 | Vosk-полировка (шум, поциферно, большая модель) | 1 | ⬜ |
| 5.9 | Room — доводка | 1 | ⬜ |
| 5.10 | Общая полировка + туториалы | 3–4 | ⬜ |
| 6 | Релиз MVP | 2 | ⬜ |

**Итого: ~12 заходов.**

---

## Известные проблемы

См. `ISSUES.md`. Открытые — уточняются.

---

## Полезные ссылки

- `AI_RULES.md` — правила работы ИИ.
- `CONTEXT_BRIEF.md` — где мы сейчас.
- `README.md` — структура.
- `DATABASE.md` — схема БД.
- `DECISIONS.md` — решения.
- `NEXT_STEPS.md` — план заходов.
- `ROADMAP.md` — общий план.
- `VOICE.md` — спецификация ГП.
- `GLOSSARY.md` — термины.
- `ISSUES.md` — проблемы.
- `TESTING.md` — тесты.