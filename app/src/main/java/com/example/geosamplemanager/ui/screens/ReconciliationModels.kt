...
/**
 * FIX 5.8.9h-2b-i-fix-1: правило filterMode.
 *
 *   • Точное совпадение (sampleNumber == q или wellNumber == q) —
 *     по ВСЕЙ базе.
 *   • Префикс (sampleNumber.startsWith(q)) — ТОЛЬКО если строка
 *     в выбранном наряде (и участке).
 *   • В обычном режиме (без фильтра) — только точное совпадение.
 */
private fun matchesQuery(
    ...
    // Префикс — только в filterMode и только для выбранного наряда.
    if (filterMode &&
...
/** Собираем VoiceSampleHit из группы UI — для передачи в UnifiedSearch. */
...
/**
 * FIX 5.8.9h-2b-i-fix-1: `analyzeMatch` для filterMode использует
 * правило «точное по базе + префикс в наряде» — то же, что
 * у `filterByQuery`. Для не-filterMode — UnifiedSearch.
 */
fun analyzeMatch(
...
/**
 * Логика для filterMode:
 *   • точное по всей базе;
 *   • префикс только для выбранного наряда.
 *
 * Если среди точных совпадений есть попадание в другой наряд/участок —
 * даём attention с причиной. Если только префикс в своём наряде — OK.
 */
private fun analyzeMatchFilterMode(
    ...
    // Группы, где есть точное совпадение.
    ...
    // matchedKind / matchedValue — из точного, если оно есть;
    // иначе из первого совпадения (префикс).
