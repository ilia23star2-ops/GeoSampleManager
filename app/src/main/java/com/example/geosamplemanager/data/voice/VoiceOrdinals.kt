...
object VoiceOrdinals {
    ...
    /**
     * FIX 5.8.6-5g: формы порядковых в родительном/творительном падеже
     * множественного числа — «четвертых», «пятых», «десятых».
     *
     * Сами по себе они не означают пробу: это либо части дробей
     * («пять четвертых», «две десятых»), либо артефакты распознавания
     * (Vosk иногда выдаёт «четвертых» вместо «четвертый»).
     *
     * Используется в VoiceCommandParser: если в фразе встречается
     * такая форма — команда считается нераспознанной, в Search не уходит.
     */
    val pluralForms: Set<String> = buildSet {
        for ((_, base) in bases) {
            add("${base}ых")
            add("${base}ыми")
        }
