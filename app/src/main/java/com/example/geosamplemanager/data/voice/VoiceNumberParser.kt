...
class VoiceNumberParser {
    ...
    fun tokenize(text: String): List<VoiceToken> {
        ...
        while (i < words.size) {
            ...
            else result.add(VoiceToken.Unknown(wClean))
            i++
        }
        return result
    }
            ...
            when (token) {
                ...
                is VoiceToken.Number -> {
                    ...
                    if (cur == null) {
                        ...
                        if (token.kind == TokenKind.D) nb.startDigitwiseWith(token.value)
                        else nb.appendNumber(token.value)
                        current = nb
                        continue
                    }
